...
public class CardRepository : BaseRepository<Card, Guid>, ICardRepository
{
    ...
    {
        const string sql = "SELECT ISNULL (MAX(Position), -1) + 1 FROM Cards WHERE ListId = @ListId;";
        return await _connection.QueryFirstOrDefaultAsync<int>(sql, new { ListId = listId }, _transaction);
    }
}